def make_webhook(project, token):
    ...
    print('webhook responder done')

def make_pm_webhook(project, token):
    import smartsheet
    import json
    import logging
    ...
    Webhook = ss_client.Webhooks.create_webhook(
        smartsheet.models.Webhook({
            'name': 'PM_Webhook_' + str(sheet.name),
            'callbackUrl': 'https://us-central1-subtle-bit-245615.cloudfunctions.net/function-2',
            'scope': 'sheet',
            'scopeObjectId': int(sheet.id),
            'events': ['*.*'],
            'version': 1}))
    ...
    # update webhook
    Webhook = ss_client.Webhooks.update_webhook(
        map_of_webhooks['PM_Webhook_' + str(sheet.name)],
        ss_client.models.Webhook({
            'enabled': True}))

    print('pm webhook done')
